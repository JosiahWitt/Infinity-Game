...
bool floorTests_types() {
  ...
  t.check(f.getColor().r == 0 && f.getColor().g == 123 / 255.0 && f.getColor().b == 12 / 255.0, "GrassFloor isn't the correct color");

  f.setFloorType(SandFloor);
  t.check(f.getColor().r == 237 / 255.0 && f.getColor().g == 201 / 255.0 && f.getColor().b == 175 / 255.0, "SandFloor isn't the correct color");

  f.setFloorType(DirtFloor);
  t.check(f.getColor().r == 120 / 255.0 && f.getColor().g == 72 / 255.0 && f.getColor().b == 0, "DirtFloor isn't the correct color");
...
bool floorTests_toJsonAndFromJson() {
  ...
  Floor f2;
  f2.fromJson(j1);
  t.check(f2.getColor().r == 0 && f2.getColor().g == 123 / 255.0 && f2.getColor().b == 12 / 255.0 && f2.getFloorType() == GrassFloor, "Didn't export and import default JSON correctly.");
  ...
  Floor f4;
  f4.fromJson(j3);
  t.check(f4.getFloorType() == DirtFloor, "Didn't export and import custom JSON correctly.");

  // Import json into an object, and assert the color, alternateColor, and
  // player's vector positions were updated correctly
  Floor f5;
  f5.fromJson({{"color", {{"r", 0.9}, {"g", 0.8}, {"b", 0.7}}}, {"floorType", 2}});
  t.check(f5.getFloorType() == SandFloor, "Didn't import custom JSON correctly.");
